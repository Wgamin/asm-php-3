<div x-show="productType === 'variable'" 
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Quản lý giá, kho và ảnh riêng cho từng loại</p>
        </div>
    ...
    <div class="space-y-4">
        ...
        <div class="p-6 border border-gray-100 rounded-2xl bg-gray-50/50 space-y-5 relative group hover:border-emerald-200 transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" :name="`variants[${index}][price]`" x-model="variant.price" required min="0" placeholder="0"
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" :name="`variants[${index}][stock]`" x-model="variant.stock" required min="0" placeholder="0"
                ...
                {{-- MỚI CẬP NHẬT: Upload ảnh riêng có Thumbnail Preview --}}
                <div class="space-y-1">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label class="cursor-pointer bg-white border border-gray-200 rounded-xl px-3 py-2 hover:bg-emerald-50 hover:border-emerald-300 flex-1 flex items-center justify-center transition group">
                            <span class="text-[11px] text-gray-500 font-bold group-hover:text-emerald-600">Chọn ảnh</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 border-t border-gray-100 pt-5">
                ...
                <div class="space-y-1">
                    ...
                    {{-- ĐÃ BỎ THUỘC TÍNH required Ở ĐÂY --}}
                    <select 
                        :name="`variants[${index}][attributes][{{ $attr->name }}]`"
                        x-model="variant.attributes['{{ $attr->name }}']"
                        class="w-full px-4 py-2 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                    >
        ...
        <div x-show="variants.length === 0" 
             x-transition
             class="text-center py-16 border-2 border-dashed border-gray-100 rounded-3xl text-gray-400 bg-gray-50/20">
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mb-4">
                    <i class="fas fa-box-open text-2xl opacity-30"></i>
                </div>
                <p class="font-bold text-gray-500">Chưa có biến thể nào</p>
                <p class="text-xs mt-1">Bấm "Thêm biến thể" để thiết lập Size, Màu sắc, Giá và Ảnh riêng</p>
            </div>
        </div>
    </div>
</div>