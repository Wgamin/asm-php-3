    ...
    <div class="container mx-auto px-4">
        @if(count($cart) > 0)
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <form id="checkout-form" action="{{ route('order.store') }}" method="POST" class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                    @csrf
                    <h2 class="text-2xl font-bold mb-6 text-gray-800">Thong tin giao hang</h2>
                    <div class="space-y-4">
                        <input type="text" name="full_name" value="{{ old('full_name', Auth::user()->name) }}" required placeholder="Ho ten" class="w-full px-5 py-3 rounded-2xl bg-gray-50 border-none border">
                        <input type="text" name="phone" value="{{ old('phone') }}" required placeholder="So dien thoai" class="w-full px-5 py-3 rounded-2xl bg-gray-50 border-none border">
                        <textarea name="address" required placeholder="Dia chi cu the" class="w-full px-5 py-3 rounded-2xl bg-gray-50 border-none border">{{ old('address') }}</textarea>
                        <textarea name="note" placeholder="Ghi chu don hang (khong bat buoc)" class="w-full px-5 py-3 rounded-2xl bg-gray-50 border-none border">{{ old('note') }}</textarea>
                        <input type="hidden" name="payment_method" value="cod">
                    </div>
                </form>

                <div class="bg-slate-800 text-white p-8 rounded-3xl shadow-xl h-fit">
                    <h2 class="text-2xl font-bold mb-6">Don hang cua ban</h2>
                            ...
                            <div class="flex justify-between items-center border-b border-slate-700 pb-4">
                                ...
                                <div class="text-right">
                                    ...
                                    <a href="{{ route('cart.remove_item', $cartKey) }}"
                                       onclick="return confirm('Xoa mon nay?')"
                                       class="text-xs text-slate-500 hover:text-red-400 mt-1 block">Xoa</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="bg-slate-900/40 rounded-2xl p-4 mb-6 border border-slate-700">
                        <p class="text-sm font-semibold mb-3">Ma giam gia</p>
                        ...
                        @else
                            <form action="{{ route('checkout.coupon.apply') }}" method="POST" class="flex flex-col sm:flex-row gap-3">
                                @csrf
                                <input type="text" name="coupon_code" value="{{ old('coupon_code') }}" placeholder="Nhap ma coupon"
        ...
        @else
            <div class="text-center py-20 bg-white rounded-3xl shadow-sm">
                <p class="text-gray-500 mb-4">Gio hang trong!</p>
                <a href="/" class="text-white bg-emerald-500 px-6 py-2 rounded-full">Mua sam ngay</a>
            </div>
        @endif
    </div>
</div>
