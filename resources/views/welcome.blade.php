...
@section('title', 'Trang chủ - Nông Sản Việt')
@section('meta_title', 'Nông Sản Việt - Nông sản sạch, tin tức hữu ích và mua sắm an tâm')
@section('meta_description', 'Khám phá nông sản sạch, sản phẩm nổi bật và tin tức hữu ích về thị trường, mẹo chọn thực phẩm tại Nông Sản Việt.')
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="flex items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-slate-100">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500">Trong vòng 2h nội thành</p>
            </div>
        </div>
        <div class="flex items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-slate-100">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500">An toàn tuyệt đối</p>
            </div>
        </div>
        <div class="flex items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-slate-100">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500">Tận tâm phục vụ</p>
            </div>
        </div>
        <div class="flex items-center gap-4 p-4 bg-white rounded-xl shadow-sm border border-slate-100">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500">Trong vòng 24h</p>
            </div>
        </div>
    </div>
</div>
...
<div class="container mx-auto px-4 pb-16">
    <div class="flex justify-between items-end mb-8 border-b border-slate-100 pb-4">
        ...
        <a href="{{ route('products.index') }}" class="group text-emerald-600 font-semibold flex items-center hover:text-emerald-700 transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
        ...
        @empty
            <div class="col-span-full py-20 text-center bg-white rounded-3xl border border-dashed border-slate-200">
                ...
                <h3 class="text-xl font-bold text-slate-800">Hiện chưa có sản phẩm</h3>
                <p class="text-slate-500 mt-2">Quay lại sau để xem những đợt mở bán nông sản mới nhất.</p>
            </div>
        @endforelse
    </div>
    ...
    @if($latestNews->isNotEmpty())
        <div class="mt-20">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8 border-b border-slate-100 pb-4">
                <div>
                    <span class="text-emerald-600 font-bold text-sm uppercase tracking-wider">Nội dung mới</span>
                    <h2 class="text-3xl font-bold text-slate-800 mt-1">Tin tức và kiến thức nổi bật</h2>
                </div>
                <a href="{{ route('news.index') }}" class="group text-emerald-600 font-semibold flex items-center hover:text-emerald-700 transition">
                    Xem tất cả bài viết
                    <i class="fas fa-arrow-right ml-2 transform group-hover:translate-x-1 transition text-sm"></i>
                </a>
            </div>
    ...
    <div class="mt-20 bg-emerald-900 rounded-3xl p-8 md:p-16 relative overflow-hidden shadow-2xl">
        <div class="relative z-10 max-w-2xl">
            ...
            <p class="text-emerald-100 mb-8 opacity-80">Đừng bỏ lỡ những đợt nông sản tươi ngon nhất vừa cập bến và các bài viết tư vấn mới nhất.</p>
